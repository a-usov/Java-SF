...
\usepackage[plain, shortend, linesnumbered, ruled]{algorithm2e}
...
\addto\extrasenglish{%
  ...
  \renewcommand{\equationautorefname}{Map}%
}
...
%
The algorithm returns $True$ only if all the types in the program are resolved, otherwise it returns $False$, meaning there is at least one type definition which is invalid and contains a cycle.
...
\begin{algorithm}[t]
    ...
    \DontPrintSemicolon
    \Input{$classes$, the set of classes in an SFJ program marked $resolved$, if their fields contain only basic types, $unresolved$ otherwise.}
...
\begin{algorithm}[t]
    \caption{Semantic Subtyping for SFJ---\emph{generateRelation}}
    \label{alg:subtyping}
    \SetKwBlock{Begin}{begin}{}
    \DontPrintSemicolon
    \Input{$classes$, the set of classes in an SFJ program.\\
    $relation$, the mapping of types to the set of subtypes, initially Map \ref{eq:initialSubtype}. \;
    }
    \Begin{
...
\setcounter{algocf}{1}
\begin{algorithm}
    \caption{Semantic Subtyping for SFJ---\emph{addClass} and \emph{checkSuperSet}}
...
%Similarly, basic types can be reduced to their largest type without consequence, such as \textbf{int and not bool} can be reduced to just \textbf{int} as we have already type checked all literal values passed to such a type.
%
