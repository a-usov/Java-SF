...
The type system of SFJ builds upon the type system by Dardha \emph{et al.} \cite{Dardha2013,Dardha2017}.
...
When visiting the AST of a program, we check if it is well-formed, following the intuition at the end of \autoref{sec:background}. We mark any classes containing fields typed with only basic types as \emph{resolved} otherwise, as \emph{unresolved}.
...
%
At each iteration of the algorithm we resolve at least one type, until all the types are resolved, and return $True$. However, if there is at least one type unresolved, that means there is a cycle in the type definition and we return $False$.

\begin{algorithm}[t]
    \SetAlgoLined
    \caption{ValidityCheck}
    \label{alg:types}

    \DontPrintSemicolon
    \Input{classes marked $resolved/unresolved$ depending if their fields contain only basic types.}
