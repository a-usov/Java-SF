...
\end{flalign*}
%
Note that \emph{Int} is not a subtype of \emph{Float} as the latter cannot represent the whole set of \emph{Int} values accurately and therefore \emph{Int} is not fully set-contained, however this is not the case for \emph{Int} and \emph{Double}. 
...
\begin{flalign*}
    &Polygon = \left\{Polygon, Triangle, Rectangle, Rumble \right\} && Triangle  = \left\{Triangle\right\}\\ 
...
\end{flalign*}
If we pass to \emph{diagonal} an argument of type \emph{Triangle}, as it is not contained in the computed set of subtypes, we get a typechecking error at compile type \cite{UD20}.
...
Similarly, given two types, the algorithm can check if one is a superset of the other, i.e.,\ they share common fields and methods, and if so, they are related by structural subtyping. 
...
Consider two structurally equivalent types $coord = [x:int, y:int, z:int]$ and $colour = [x:int, y:int, z:int]$. 
While they can be used interchangeably in our type system, the meaning of the types is completely different, meaning we ignore possible wrong behaviour in our program. 
